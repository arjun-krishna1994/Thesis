...
def check_ac():
    ...
    sensor_processor = restlets.SensorRestlet({'time': [30, datetime.datetime.now() + timedelta(seconds=400), 'time_var',] })
    sensor_processor.process_sensor_input(0, True)        
    reactor.callLater(10, check_ac)
...
class SensorAgent():
    ...
    def processLaterResponse(self, response):
        import json 
        import datetime
        f = open('actuator_sensor_end_1.txt', 'a+')
        ...
        from restlets import restlets
        sensor_processor = restlets.SensorRestlet({'time': [30, datetime.datetime.now() + timedelta(seconds=400), 'time',] })
