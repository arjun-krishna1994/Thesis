...
class CloudAgent():
    ...
    def processResponse(self, response):
        print colored('Connection to cloud sensor endpoint at ' + self.host + ':' + str(self.port) + "/" + self.uri +  ' established by actuator', 'red' )

    def processLaterResponse(self, response):
        #Record the end time for the packet
        import json 
        import datetime
        f = open('actuator_cloud_end_%s.txt'%(self.sensor_number,), 'a+')
        ...
        print "Processed Observe input from sensor " + self.sensor_number
...
import os
remote_hst = os.environ['rmt_host']
endpoint = resource.Endpoint(None)
protocol = coap.Coap(endpoint)
client = CloudAgent(protocol, host=remote_hst, port=5684, uri='sensorReading1')
client2 = CloudAgent(protocol, host=remote_hst, port=5684, uri='sensorReading2')
client3 = CloudAgent(protocol, host=remote_hst, port=5684, uri='sensorReading3')
reactor.listenUDP(61657, protocol)
reactor.run()
